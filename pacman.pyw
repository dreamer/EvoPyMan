...
import pygame, sys, os, random, instruction
		...
		if (player.velX, player.velY) == (0,0) or thisLevel.CheckIfHitWall((player.x + player.velX, player.y + player.velY), (player.nearestRow, player.nearestCol)):
			...
			# while True:
				...
				if dir == 0 and not thisLevel.CheckIfHitWall((player.x + player.speed, player.y), (player.nearestRow, player.nearestCol)):
					...
					player.velY = 0
					break
				elif dir == 1 and not thisLevel.CheckIfHitWall((player.x - player.speed, player.y), (player.nearestRow, player.nearestCol)):
					player.velX = -player.speed
					player.velY = 0
					break
				elif dir == 2 and not thisLevel.CheckIfHitWall((player.x, player.y + player.speed), (player.nearestRow, player.nearestCol)):
					player.velX = 0
					player.velY = player.speed
					break
				elif dir == 3 and not thisLevel.CheckIfHitWall((player.x, player.y - player.speed), (player.nearestRow, player.nearestCol)):
					player.velX = 0
					player.velY = -player.speed
					break
...
def StateMatch(cell, (bigDot, dot, ghost)):
	...
	if cell.element == "Ghost":
		if ghost[0] >= cell.range[0] and ghost[0] <= cell.range[1] and instruction.DirModel[ghost[1]] > cell.dirItIs:
			return True
		return False
	if cell.element == "SmallDot":
		if dot[0] >= cell.range[0] and dot[0] <= cell.range[1] and instruction.DirModel[dot[1]] > cell.dirItIs:
			return True
		return False
	if cell.element == "BigDot":
		if bigDot[0] >= cell.range[0] and bigDot[0] <= cell.range[1] and instruction.DirModel[bigDot[1]] > cell.dirItIs:
			return True
		return False
...
def distance():
  x = (player.x  + 8) / 16
  y = (player.y  + 8) / 16
  (dot_R,turbo_dot_R,ghosts_R) = find_distance(x+1,y,10,"R")
  (dot_L,turbo_dot_L,ghosts_L) = find_distance(x-1,y,10,"L")
  (dot_U,turbo_dot_U,ghosts_U) = find_distance(x,y+1,10,"U")
  (dot_D,turbo_dot_D,ghosts_D) = find_distance(x,y-1,10,"D")
  min_turbo_dot = min((turbo_dot_R,"R"),(turbo_dot_L,"L"),(turbo_dot_U,"U"),(turbo_dot_D,"D"))
  min_dot = min((dot_R,"R"),(dot_L,"L"),(dot_U,"U"),(dot_D,"D"))
  min_ghost = min((min(ghosts_R),"R"),(min(ghosts_L),"L"),(min(ghosts_U),"U"),(min(ghosts_D),"D"))
  return(min_turbo_dot,min_dot,min_ghost)

def find_distance(begin_x,begin_y,begin_depth,begin_direct):
  queue = Queue.Queue(0)
  min_dot = begin_depth+1
  min_turbo_dot = begin_depth+1
  min_ghost = [begin_depth+1,begin_depth+1,begin_depth+1,begin_depth+1]
  queue.put((begin_x,begin_y, begin_direct,begin_depth))
  while not(queue.empty()):
    (x,y,direct,depth) = queue.get_nowait()
    map_elem = thisLevel.map[thisLevel.lvlWidth*y+(x % thisLevel.lvlWidth)]
    if map_elem==2:
      min_dot = min(min_dot,begin_depth-depth)
    if map_elem==3:
      min_turbo_dot = min(min_turbo_dot,begin_depth-depth)
    for i in range(0, 4, 1):
      if int(((ghosts[i].y + 8) / 16))==y and int(((ghosts[i].x + 8) / 16))==x:
        min_ghost[i] = min(min_ghost[i],begin_depth-depth)
    if depth>0 and (map_elem==2 or map_elem==3 or map_elem==0):
      if direct!='L':
        queue.put((x+1,y, "R",depth-1))
      if direct!='R':
        queue.put((x-1,y, "L",depth-1))
      if direct!='D':
        queue.put((x,y+1, "U",depth-1))
      if direct!='G':
        queue.put((x,y-1, "D",depth-1))
  return (min_dot,min_turbo_dot,min_ghost);
